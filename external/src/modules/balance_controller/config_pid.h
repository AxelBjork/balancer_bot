...
// ====== Motor / speed ceiling (primary scaling knob) ======
  ...
  static constexpr double max_sps           = 5000.0;      // clamp for wheel speed command (steps/s)
  ...
  static constexpr double rate_P      = 0.80;  // try 0.12–0.30
  static constexpr double rate_I      = 0.0018;  // keep 0 while tuning
  static constexpr double rate_D      = 0.10;  // keep 0 while tuning
  static constexpr double rate_I_lim  = 0.15;  // unused when I=0
  static constexpr double rate_FF     = 0.06;  // usually 0 for balancing
