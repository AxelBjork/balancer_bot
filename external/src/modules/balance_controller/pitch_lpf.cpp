...
double PitchComplementaryFilter::rad2deg(double x) { return x * (180.0 / M_PI); }
...
double PitchComplementaryFilter::acc_pitch(const Acc3& a) {
  ...
  return std::atan2(-ax, std::sqrt(ay*ay + az*az)); // your convention
}
...
void PitchComplementaryFilter::publish(double pitch, double gyro_pitch, double yaw_rate_z, TimePoint ts) {
  uint64_t s = seq_.load(std::memory_order_relaxed);
