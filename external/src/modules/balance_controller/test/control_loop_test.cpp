...
public:
  ...
  bool waitSizeAtLeast(
      size_t n, std::chrono::milliseconds to = std::chrono::milliseconds(600)) {
...
template <class CtrlT> class ImuFeeder {
public:
  ...
  void stop() {
    bool exp = true;
    if (running_.compare_exchange_strong(exp, false) && th_.joinable())
      th_.join();
  }
...
protected:
  ...
  bool
  waitTelemetry(size_t n,
                std::chrono::milliseconds to = std::chrono::milliseconds(600)) {
...
static std::vector<ImuSample> loadImuCsv(const std::string &path) {
  std::ifstream f(path);
  if (!f)
    throw std::runtime_error("Couldn't open CSV: " + path);
  ...
  // Accept exact header; ignore whitespace
  {
    ...
    std::getline(iss, h2, ',');
    std::getline(iss, h3, ',');
    if (trim(h0) != "pitch" || trim(h1) != "dpitch" || trim(h2) != "yaw" ||
        trim(h3) != "time") {
      throw std::runtime_error(
          "Unexpected CSV header; expected: pitch,dpitch,yaw,time");
    }
  }
  ...
  while (std::getline(f, line)) {
    ...
    if (!std::getline(iss, c0, ','))
      continue;
    if (!std::getline(iss, c1, ','))
      continue;
    if (!std::getline(iss, c2, ','))
      continue;
    if (!std::getline(iss, c3, ','))
      continue;
    try {
      rows.push_back(
          Row{std::stod(c0), std::stod(c1), std::stod(c2), std::stod(c3)});
  ...
  for (const auto &r : rows) {
    ...
    s.t = t0 + std::chrono::duration_cast<clk::duration>(
...
TEST(CascadedControllerCsvReplayTest, ReplaysCsvAndPrintsTelemetry) {
  ...
  ctrl.setTelemetrySink([&](const Telemetry &t) {
    int n = ++tel_count;
    if (kPrintEvery > 0 && (n % kPrintEvery) != 0)
      return;
    constexpr double deg = 180.0 / M_PI;
    std::printf("t=%7.3f  θ=%6.2f°  θ̇=%6.2f°/s  aθ=%7.1f  aθ̇=%7.1f  av=%7.1f  "
                "u=%6.0f%s  L=%6.0f  R=%6.0f\n",
                std::chrono::duration<double>(t.ts.time_since_epoch()).count(),
                t.tilt_rad * deg, t.gyro_rad_s * deg, t.a_theta_mps2,
                t.a_dtheta_mps2, t.a_v_mps2, t.u_balance_sps,
