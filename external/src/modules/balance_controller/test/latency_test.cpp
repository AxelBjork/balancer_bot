...
TEST(DataPathSanity, ComplementaryFilter_RiseTime_20DegStep) {
  ...
  const auto tick =
      std::chrono::nanoseconds{(long long)std::llround(1e9 / fs_hz)};
  ...
  const double tau_corr =
      1.0 / (2.0 * M_PI * std::max(0.1, Config::fc_acc_corr_hz));
  const double tau_gyro =
      1.0 / (2.0 * M_PI * std::max(0.1, Config::fc_gyro_lpf_hz));
  ...
  auto accel_for_pitch_g = [](double pitch_rad) {
    const double s = std::sin(pitch_rad), c = std::cos(pitch_rad);
    return std::array<double, 3>{-s * Config::g0, 0.0,
                                 c * Config::g0}; // atan2(-ax, az)
  };
  ...
  auto push = [&](double pitch_true, double gyro_y_rps) -> ImuSample {
    now += tick;
    filt.push_sample(accel_for_pitch_g(pitch_true), {0.0, gyro_y_rps, 0.0},
                     now);
  ...
  const double tr_ms =
      (k10 > 0 && k90 > 0) ? (t90_ms - t10_ms) : (maxN * dt_s * 1e3);
  std::printf("[Pitch rise] step=%.1f° fs=%.0fHz k10=%d (%.1f ms) k90=%d (%.1f "
  ...
  EXPECT_LE(tr_ms, Config::pitch_rise_ms)
      << "Accel-correction path is too slow for control needs.";
}
...
TEST(DataPathSanity, ComplementaryFilter_GyroLPF_RiseTime_RateStep) {
  ...
  const double tr_ms =
      (k10 > 0 && k90 > 0) ? (k90 - k10) * dt_s * 1e3 : maxN * dt_s * 1e3;
  std::printf("[Gyro rise] step=%g°/s fs=%.0fHz k10=%d k90=%d rise=%.1f ms "
              "(bound=%d)\n",
  ...
  EXPECT_LE(tr_ms, Config::dpitch_rise_ms)
      << "Gyro path is too slow for control needs.";
}