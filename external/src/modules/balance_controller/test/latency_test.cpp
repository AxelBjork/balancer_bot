...
TEST(DataPathSanity, StepperStepN_ManyDirFlips_NoCumulativeMilliseconds) {
  ...
  EXPECT_LT(us, 5000) << "Cumulative latency suspiciously high: " << us << " us";
}
...
TEST(DataPathSanity, StepperStepN_DirFlip_DoesNotAddMilliseconds) {
  ...
  EXPECT_LT(second_us, 300) << "Dir-flip stepN too slow: " << second_us << " us";
}
...
TEST(DataPathSanity, ComplementaryFilter_RiseTime_20DegStep) {
  ...
  std::printf("[Pitch rise] step=%.1f° fs=%.0fHz k10=%d (%.1f ms) k90=%d (%.1f ms) rise=%.1f ms (bound=%d)\n",
...
TEST(DataPathSanity, ComplementaryFilter_GyroLPF_RiseTime_RateStep) {
  ...
  std::printf("[Gyro rise] step=%g°/s fs=%.0fHz k10=%d k90=%d rise=%.1f ms (bound=%d)\n",
