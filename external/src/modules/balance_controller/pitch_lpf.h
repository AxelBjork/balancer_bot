...
public:
  ...
  void push_sample(const Acc3& acc, const Gyr3& gyrv, TimePoint ts) {
    ...
    // 1) Prefilter accel
    for (int i = 0; i < 3; ++i) acc_f_[i] = a_acc * acc_f_[i] + (1.0 - a_acc) * acc[i];
  ...
  void reset() {
    init_ = false; have_last_ts_ = false;
    acc_f_ = {0.0,0.0,0.0}; pitch_ = 0.0; gyro_lpf_ = 0.0; gyro_bias_ = 0.0;
    uint64_t s = seq_.load(std::memory_order_relaxed);
    seq_.store(s+1, std::memory_order_release); pub_ = ImuSample{}; seq_.store(s+2, std::memory_order_release);
  }
...
private:
  ...
  static inline double acc_pitch(const Acc3& a) {
    ...
    return std::atan2(-ax, std::sqrt(ay*ay + az*az)); // matches your convention
  }
  ...
  bool accel_reliable(const Acc3& a, double pitch_pred) const {
    ...
    if (std::abs(rad2deg(std::cos(pitch_pred))) < 1e-6) return false; // avoid divide-by-zero edge cases
    if (std::abs(rad2deg(pitch_pred)) > Config::max_use_pitch_deg) return false; // near +/-90°, accel pitch is ill-conditioned
    return true;
  }
