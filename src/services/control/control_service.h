...
inline constexpr char kControlServiceDoc[] =
    ...
    "`JoystickCommand`, and `MotorFeedback` inputs into actuator commands and streaming "
    "controller "
    "telemetry.\n\n"
    ...
    "controller. A joystick forward command requests longitudinal acceleration. The outer loop "
    "jerk-limits that request, subtracts velocity damping, and maps the resulting acceleration "
    "to a pitch reference; lean trim is then added. The PX4 `RateControl` block tracks a "
    "damped pitch-rate setpoint:\n\n"
