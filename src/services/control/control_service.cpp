...
ControlService::ControlService(ipc::MessageBus& bus) : bus_(bus) {
  ...
  core_.setTelemetrySink([this](const Telemetry& t) {
    ...
    p.target_velocity_sps = static_cast<float>(t.target_vel_sps);
    p.vel_error = static_cast<float>(t.vel_error);
    p.measured_vel_sps = static_cast<float>(t.measured_vel_sps);
    p.velocity_p_term_deg = static_cast<float>(t.vel_p_term_deg);
    p.velocity_i_term_deg = static_cast<float>(t.vel_i_term_deg);
    ...
    p.left_applied_sps =
        static_cast<float>(have_motor_feedback_ ? latest_motor_feedback_.left_applied_sps : last_left_sps_);
    p.right_applied_sps =
        static_cast<float>(have_motor_feedback_ ? latest_motor_feedback_.right_applied_sps : last_right_sps_);
